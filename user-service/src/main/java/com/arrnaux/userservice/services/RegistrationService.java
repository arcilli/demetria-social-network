...
public class RegistrationService {
    ...
    public boolean registerUser(@RequestBody SNUserRegistrationDTO snUserRegistrationDTO) throws Exception {
        logger.info("Trying to register with info: " + snUserRegistrationDTO);
        // TODO: add a check for not-null user properties
        Optional<SNUser> user = snUserRepository.findByEmail(snUserRegistrationDTO.getEmail());
        if (user.isPresent()) {
            ...
            snUserRepository.save(new SNUser(snUserRegistrationDTO));
            logger.info("Register with info: " + snUserRegistrationDTO);
            return true;
        }
    }
